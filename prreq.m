// The link to your repo (must be created Feb1 or after)
// Post the repo to the class slack
A list of five issues must exist in the repo.
    1. trying to figure out how to code com players
        a. random functions for their cards
        b. logic for card pulling 
        c. andrina method
Defined contribution guidelines and instructions for how to contribute/run/test
Defined unit tests run with each checkin
Coding guidelines and standards
Readme or docs on how to run the application and test
The required configuration to force Pull Requests to go through a review process.
 